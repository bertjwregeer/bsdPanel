...
public class dnsdata {
	...
	public static void main(final String[] pArgs) throws IOException {
		...
		if (pArgs.length < 1) {
			System.out.println("Usage: java DNSClient datafile");
			System.exit(0);
		}
		iFile = pArgs[0];
	...
	private static void createNS(final char type) throws IOException {
		...
		// construct string
		StringBuffer entry = new StringBuffer("" + type + tFQDN + DNSEntry.delimiter() 
			+ tIP + DNSEntry.delimiter() + tX + DNSEntry.delimiter() + tTTL + 
			DNSEntry.delimiter() + tTS + DNSEntry.delimiter() + tLO);
	...
	private static void createHost(final char type) throws IOException {
		...
		// construct string
		StringBuffer entry = new StringBuffer("" + type + tFQDN + DNSEntry.delimiter() 
			+ tIP + DNSEntry.delimiter() + tTTL + DNSEntry.delimiter() + tTS + 
			DNSEntry.delimiter() + tLO);
		while (entry.charAt(entry.length() - 1) == ':') {
			entry.deleteCharAt(entry.length() - 1);
		}
	...
	private static void createMX() throws IOException {
		...
		// construct string
		StringBuffer entry = new StringBuffer("" + '@' + tFQDN + DNSEntry.delimiter() 
			+ tIP + DNSEntry.delimiter() + tX + DNSEntry.delimiter() + tDist + 
			DNSEntry.delimiter() + tTTL + DNSEntry.delimiter() + tTS + DNSEntry.delimiter() + tLO);
		while (entry.charAt(entry.length() - 1) == ':') {
			entry.deleteCharAt(entry.length() - 1);
		}
